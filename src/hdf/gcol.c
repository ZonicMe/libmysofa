...
static int readGCOL(struct READER *reader) {
  ...
  /* read signature */
  if (readfn(reader->fhd, buf, 4, reader->file) != 4 || strncmp(buf, "GCOL", 4)) {
    mylog("cannot read signature of global heap collection\n");
    return MYSOFA_INVALID_FORMAT;
  }
  buf[4] = 0;

  if (getcfn(reader->fhd, reader->file) != 1) {
    mylog("object GCOL must have version 1\n");
    return MYSOFA_INVALID_FORMAT;
  }
  if (getcfn(reader->fhd, reader->file) < 0 || getcfn(reader->fhd, reader->file) < 0 ||
      getcfn(reader->fhd, reader->file) < 0)
    return MYSOFA_READ_ERROR;

  address = tellfn(reader->fhd, reader->file);
  end = address;
  ...
  while (tellfn(reader->fhd, reader->file) <= end - 8 - reader->superblock.size_of_lengths) {
    ...
    if (seekfn(reader->fhd, 4, SEEK_CUR, reader->file) < 0) {
      free(gcol);
      return errno;
    }
...
int gcolRead(struct READER *reader, uint64_t gcol, int reference,
  ...
  if (!p) {
    pos = tellfn(reader->fhd, reader->file);
    if (seekfn(reader->fhd, gcol, SEEK_SET, reader->file) < 0)
      return MYSOFA_READ_ERROR;
    readGCOL(reader);
    if (pos < 0)
      return MYSOFA_READ_ERROR;
    if (seekfn(reader->fhd, pos, SEEK_SET, reader->file) < 0)
      return MYSOFA_READ_ERROR;
