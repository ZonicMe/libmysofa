...
static int readBTLF(struct READER *reader, struct BTREE *btree,
  ...
  /* read signature */
  if (readfn(reader->fhd, buf, 4, reader->file) != 4 || strncmp(buf, "BTLF", 4)) {
  ...
  type = (uint8_t)getcfn(reader->fhd, reader->file);
...
int btreeRead(struct READER *reader, struct BTREE *btree) {
  ...
  /* read signature */
  if (readfn(reader->fhd, buf, 4, reader->file) != 4 || strncmp(buf, "BTHD", 4)) {
  ...
  btree->type = (uint8_t)getcfn(reader->fhd, reader->file);
  ...
  btree->split_percent = (uint8_t)getcfn(reader->fhd, reader->file);
  btree->merge_percent = (uint8_t)getcfn(reader->fhd, reader->file);
  ...
  /* read records */
  if (seekfn(reader->fhd, btree->root_node_address, SEEK_SET, reader->file) < 0)
    return errno;
...
int treeRead(struct READER *reader, struct DATAOBJECT *data) {
  ...
  /* read signature */
  if (readfn(reader->fhd, buf, 4, reader->file) != 4 || strncmp(buf, "TREE", 4)) {
  ...
  node_type = (uint8_t)getcfn(reader->fhd, reader->file);
  node_level = (uint8_t)getcfn(reader->fhd, reader->file);
  ...
  for (e = 0; e < entries_used * 2; e++) {
    if (node_type == 0) {
      ...
      if (seekfn(reader->fhd, child_pointer, SEEK_SET, reader->file) < 0) {
        free(output); // LCOV_EXCL_LINE
      ...
      if (readfn(reader->fhd, input, size_of_chunk, reader->file) != size_of_chunk) {
        free(output);                 // LCOV_EXCL_LINE
