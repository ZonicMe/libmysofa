...
int superblockRead2or3(struct READER *reader, struct SUPERBLOCK *superblock) {
  ...
  if (getcfn(reader->fhd, reader->file) < 0) /* File Consistency Flags */
  ...
  if (seekfn(reader->fhd, 0L, SEEK_END, reader->file))
    return errno;

  if (superblock->end_of_file_address != tellfn(reader->fhd, reader->file)) {
...
int superblockRead0or1(struct READER *reader, struct SUPERBLOCK *superblock,
                       int version) {
  if (getcfn(reader->fhd, reader->file) !=
  ...
  superblock->size_of_offsets = (uint8_t)getcfn(reader->fhd, reader->file);
  superblock->size_of_lengths = (uint8_t)getcfn(reader->fhd, reader->file);
...
int superblockRead(struct READER *reader, struct SUPERBLOCK *superblock) {
  ...
  /* signature */
  if (readfn(reader->fhd, buf, 8, reader->file) != 8 ||
      strncmp("\211HDF\r\n\032\n", buf, 8)) {
